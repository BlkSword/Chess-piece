...
fn get_security_directory(data: &[u8]) -> Result<(u32, u32), String> {
    ...
    let virt_addr =
        u32::from_le_bytes(data[sec_dir_offset..sec_dir_offset + 4].try_into().unwrap());
    let size = u32::from_le_bytes(
        data[sec_dir_offset + 4..sec_dir_offset + 8]
            .try_into()
            .unwrap(),
    );
...
fn update_security_directory(path: &Path, offset: u32, size: u32) -> Result<(), String> {
    ...
    let opt_header_offset = e_lfanew + 24;
    let magic = u16::from_le_bytes(
        buf[opt_header_offset..opt_header_offset + 2]
            .try_into()
            .unwrap(),
    );
    ...
    file.seek(SeekFrom::Start(sec_dir_offset as u64))
        .map_err(|e| e.to_string())?;
